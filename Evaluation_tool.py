...
class EvaluationTool:
    ...
    def __init__(self):
        ...
        self.true_CP_location = self.true_CP_model_index = self.true_CP_model_label = (
            None
        )
    ...
    *********************************************************************"""
    ...
    def build_EvaluationTool_models_via_specs(
        self, model_class_list, specification_list, data=None, detector_specs=None
    ):
        ...
        *specification_list* will give you the list of lists of parameters for
        ...
        self.model_universe = []
        for model_class, specs in zip(self.model_class_list, self.specification_list):
    ...
    def build_EvaluationTool_models_via_universe(
        self, model_universe, data=None, detector_specs=None
    ):
    ...
    def build_EvaluationTool_models_via_csv(
        self, specification_csv, data=None, detector_specs=None
    ):
    ...
    *********************************************************************"""
    ...
    def build_EvaluationTool_via_run_detector(
        self,
        detector,
        true_CP_location=None,
        true_CP_model_index=None,
        true_CP_model_label=None,
    ):

        if (true_CP_location is not None) and (true_CP_model_index is not None):
            self.add_true_CPs(
                true_CP_location, true_CP_model_index, true_CP_model_label
            )
    ...
    *********************************************************************"""
    ...
    def add_data_and_detector_via_hand(self, data, detector_specs=None):
        ...
        if self.type is None or self.type < 2:
            print(
                "Error! Passing in data and detector specs before "
    ...
    def create_data_add_detector_via_simulation_obj(
        self, sim_class, sim_parameters, detector_specs=None
    ):
    ...
    def create_data_add_detector_via_simulation_csv(self, sim_csv, detector_specs=None):
        """You run a simulation *sim_class* that takes parameters stored in
        *sim_parameters* and creates a detector afterwards. It is essential
        that the sim_class object stores the generated data as ".data" """
    ...
    def add_true_CPs(
        self, true_CP_location, true_CP_model_index, true_CP_model_label=None
    ):
        ...
        self.results[self.results[0].index("true CP model index")] = (
            self.true_CP_model_index
        )
        self.results[self.results[0].index("true CP model labels")] = (
            self.true_CP_model_label
        )
    ...
    *********************************************************************"""
    ...
    def store_detector_results_to_object(self):
        if self.type < 4:
            print(
                "CAREFUL! Your detector seems to not have been run yet, "
        ...
        self.model_and_run_length_log_distr = (
            self.detector.model_and_run_length_log_distr
        )
        ...
        for i in range(0, M):
            class_label = (
                str(type(self.detector.model_universe[i])).split(".")[-1].split("'")[0]
            )
    ...
    *********************************************************************"""
    ...
    def create_BVAR_configurations_easy(
        num_configs,
        a,
        b,
        prior_mean_beta,
        ...
        S1, S2 = EvaluationTool.create_args(
            num_configs, S1
        ), EvaluationTool.create_args(num_configs, S2)
        nbh_sequence = EvaluationTool.create_args(num_configs, nbh_sequence)
        restriction_sequence = EvaluationTool.create_args(
            num_configs, restriction_sequence
        )
        intercept_grouping = EvaluationTool.create_args(num_configs, intercept_grouping)
        general_nbh_sequence = EvaluationTool.create_args(
            num_configs, general_nbh_sequence
        )
        ...
        for i in range(0, num_configs):
            ...
            configs[i] = [
                ...
                S2[i],
                nbh_sequence[i],
                restriction_sequence[i],
                intercept_grouping[i],
                general_nbh_sequence[i],
                general_nbh_restriction_sequence[i],
                nbh_sequence_exo[i],
                exo_selection[i],
                padding[i],
                auto_prior[i],
    ...
    def create_BVAR_configurations(
        a_list,
        b_list,
        prior_mean_beta_list,
        prior_var_beta_list,
        S1_list,
        S2_list,
        nbh_sequence_list,
        restriction_sequence_list,
        intercept_grouping_list=None,
        general_nbh_sequence_list=None,
        general_nbh_restriction_sequence_list=None,
        nbh_sequence_exo_list=None,
        exo_selection_list=None,
        padding_list=None,
        auto_prior_list=None,
    ):
        ...
        for i in range(0, num_configs):
            configs[i] = [
                a_list[i],
                b_list[i],
                prior_mean_beta_list[i],
                prior_var_beta_list[i],
                S1_list[i],
                S2_list[i],
                nbh_sequence_list[i],
                restriction_sequence_list[i],
                intercept_grouping_list[i],
                general_nbh_sequence_list[i],
                general_nbh_restriction_sequence_list[i],
                nbh_sequence_exo_list[i],
                exo_selection_list[i],
                padding_list[i],
                auto_prior_list[i],
            ]
    ...
    *********************************************************************"""
    ...
    def plot_raw_TS(
        ...
        for i in range(0, num):  # num = len(indices)
            ...
            handle = ax.plot(
                x_axis[:up_to], data[:up_to, indices[i]], color=custom_colors_series[i]
            )
        ...
        if (
            False
        ):  # DEBUG: We need to add CP option self.results[self.names.index("has true CPs")]:
            CP_legend_labels = []
            CP_legend_handles = []
            ...
            for CP_loc, CP_ind, CP_lab in zip(
                CP_locations, CP_model_index, CP_model_labels
            ):
                handle = ax.axvline(
                    x=CP_loc, color=self.CP_color, linestyle=self.linestyle[CP_ind]
                )
        ...
        if additional_CPs is not None:
            ...
            if custom_linestyles_additional_CPs is None:
                custom_linestyles_additional_CPs = (
                    self.linestyle
                )  # ['solid']*len(CP_locations)
                ...
                for CP_loc, CP_ind in zip(CP_locations, CP_indices):
                    if m == CP_ind:
                        if CP_loc <= time_range[-1] and CP_loc >= time_range[0]:
                            CP_loc = ((CP_loc - time_range[0]) / T_) * (
                                stop - start
                            ) + start  # carry CP forward
                            if CP_ind not in CP_indices_until_now:
                                handle = ax.axvline(
                                    x=CP_loc,
                                    color=custom_colors_additional_CPs[count],
                                ...
                                handle = ax.axvline(
                                    x=CP_loc,
                                    color=custom_colors_additional_CPs[relevant_index],
                                    linestyle=custom_linestyles_additional_CPs[
                                        relevant_index
                                    ],
                                    linewidth=custom_linewidth_additional_CPs,
                                    alpha=custom_transparency_additional_CPs,
        ...
        if show_MAP_CPs:
            ...
            if up_to == len(data[:, 0]):
                ...
                if (
                    len(self.results[self.names.index("MAP CPs")][up_to]) == 0
                    and up_to < len(self.results[self.names.index("MAP CPs")]) - 2
                ):
                    ...
                    while bool_:
                        count = count + 1
                        if len(self.results[self.names.index("MAP CPs")][count]) > 0:
                            bool_ = False
                    CP_object = self.results[self.names.index("MAP CPs")][count]
                elif up_to >= len(self.results[self.names.index("MAP CPs")]) - 2:
                    ...
                    if m == CP_ind:
                        if CP_loc <= time_range[-1] and CP_loc >= time_range[0]:
                            ...
                            if CP_ind not in CP_indices_until_now:
                                ...
                                handle = ax.axvline(
                                    x=CP_loc,
                                    color=custom_colors_CPs[relevant_index],
        ...
        if not true_CPs is None:
            ...
            for entry in true_CPs:
                ax.axvline(
                    x=entry[0], color=entry[1], linestyle="-", linewidth=entry[2]
                )
    ...
    def plot_predictions(
        ...
        S1, S2 = (
            self.results[self.names.index("S1")],
            self.results[self.names.index("S2")],
        )
        means = (
            self.results[self.names.index("one-step-ahead predicted mean")][
                time_range - 1, :, :
            ]
        ).reshape((int(len(time_range)), S1 * S2))[:, indices]
        if show_var:
            std_err = np.sqrt(
    ...
    def plot_prediction_error(
        ...
        pred = (
            self.results[self.names.index("one-step-ahead predicted mean")]
        ).reshape(T, S1 * S2)[time_range - 1, :][:, indices]
        ...
        for i in range(0, num):
            """The handle is like an identifier for that TS object"""
            (handle,) = ax.plot(
                time_range[:up_to], pred_errors[:up_to, i], color=custom_colors[count]
            )
            count = count + 1
            legend_handles.append(handle)
            """If required, also plot the errors around the series"""
            if show_var:
                ax.plot(
                    time_range[:up_to],
                    pred_errors[:up_to, i] + std_err[:up_to, i],
    ...
    def plot_model_posterior(
        ...
        lengths"""
        m_rl_distr = self.results[
            self.names.index("all model and run length log distributions")
        ]
        M = (m_rl_distr[-1][:, 0]).shape[0]
        # DEBUG: offset should be smallest lag length if starting point is smaller
        #       than smallest lag length
        offset = max(0, (np.size(time_range) - len(m_rl_distr)))
        ...
        for t, i in zip(
            range(start + offset, stop), range(0, np.size(time_range))
        ):  # m_rl_distr[time_range]:
            for m in range(0, M):
                if m < m_rl_distr[t - offset][:, :].shape[0]:
                    model_posterior[m, i] = logsumexp(m_rl_distr[t - offset][m, :])
        if not log_format:
        ...
        if plot_type == "trace":
            ...
            handle = ax.plot(
                x_axis[:up_to], MAPs[:up_to], linewidth=3.0, color=custom_colors[0]
            )
            ...
            plot_type == "MAPVariance1_trace" or plot_type == "MAPVariance1_det"
        ):  # MAPVariance1_det
            ...
            for t in range(0, window_len):
                map_variances[:, t] = np.var(
                    model_posterior[indices, : (t + 1)].reshape(len(indices), t + 1),
                    axis=1,
                )
                if plot_type == "MAPVariance1_det":
                    minval = max(t + 1, len(indices) + 1)
                    covs = np.cov(model_posterior[indices, :minval])
            ...
            for t in range(window_len, len(time_range)):
                map_variances[:, t] = np.var(
                    model_posterior[indices, (t - window_len) : t], axis=1
                )
                if plot_type == "MAPVariance1_det":
                    covs = np.cov(model_posterior[indices, (t - window_len) : t])
                    deleted_indices = np.all(np.abs(covs) > eps, axis=1)
                    covs = NPD.nearestPD(covs[~deleted_indices][:, ~deleted_indices])
                    sign, ldet = np.linalg.slogdet(covs)
                    map_cov_dets[t] = sign * np.exp(ldet)
        ...
        elif plot_type == "MAPVariance2_trace" or plot_type == "MAPVariance2_det":
            """Plot map variance by considering variance about the
            ...
            for t in range(0, window_len):
                ...
                frequencies = np.array(
                    [
                        collections.Counter(MAPs[: (t + 1)])[i] / (t + 1)
                        for i in range(0, len(indices))
                    ]
                )
                """STEP 2: Calcuate MVN vars from that"""
                MVN_variance[t] = np.sum([f * (1 - f) * (t + 1) for f in frequencies])
            ...
            for t in range(window_len, len(time_range)):
                ...
                frequencies = np.array(
                    [
                        collections.Counter(MAPs[(t - window_len) : t])[i] / window_len
                        for i in range(0, len(indices))
                    ]
                )
                """STEP 2: Calcuate MVN vars from that"""
                MVN_variance[t] = np.sum(
                    [f * (1 - f) * window_len for f in frequencies]
                )
                """STEP 3: calculate covariances (MVN off-diagonals)"""
                if plot_type == "MAPVariance2_det":
                    ...
                    covs[diag_ind] = MVN_variance[t]
                    deleted_indices = np.all(covs == 0, axis=1)
                    covs = covs[~deleted_indices][
                        :, ~deleted_indices
                    ]  # remove all 0-rows/cols
                    MVN_cov_dets[t] = np.linalg.det(covs)
                    if SGV:
                        MVN_cov_dets[t] = pow(MVN_cov_dets[t], covs.shape[0])
            """Plot"""
            ...
            if not log_format:
                BF = (
                    model_posterior[indices[0], :up_to]
                    / model_posterior[indices[1], :up_to]
                )
            else:
                BF = (
                    model_posterior[indices[0], :up_to]
            ...
            if True:
                handle = ax.plot(
                    x_axis[up_to], BF, linewidth=3.0, color=custom_colors[0]
                )
                # gray shading
                ax.fill_between(
                    x=[x_axis[0], x_axis[-1]],
        ...
        if show_MAP_CPs:
            ...
            for CP_loc, CP_ind in zip(CP_locations, CP_indices):
                if CP_loc <= time_range[-1] and CP_loc >= time_range[0]:
                    ...
                    handle = ax.axvline(
                        x=CP_loc, color=self.CP_color, linestyle=self.linestyle[count]
                    )
                    if CP_ind not in CP_indices_until_now:
                        CP_legend_handles.append(handle)
                        CP_legend_labels.append(model_labels[CP_ind])
        ...
        if not period_time_list is None and not label_list is None:
            ...
            for period, label in zip(period_time_list, label_list):
                ...
                ax.annotate(
                    label,
                    xytext=(stop_period + number_offset, ypos - text_offset),
                    xycoords="data",
    ...
    def plot_run_length_distr(
        ...
        """STEP 1: Default is to take the entire time range"""
        T = self.results[self.names.index("T")]
        storage_run_length_log_distr = self.results[
            self.names.index("all run length log distributions")
        ]
        ...
        if time_range is None:
            ...
            time_range = np.linspace(
                offset, time_range[-1], time_range[-1] - offset, dtype=int
            )
        ...
        if orientation == "horizontal" and colorbar_location is not None:
            cax = divider.append_axes(
                colorbar_location, size="5%", pad=space_to_colorbar
            )
        ...
        r_max = 0
        storage_all_retained_run_lengths = self.results[
            self.names.index("all retained run lengths")
        ]
        ...
        for t, ind in zip(time_range - 1, range(0, up_to)):  # range(offset, T_rl):
            ...
            if log_format:
                ...
                if no_transform:
                    ...
                    pdfs[retained_run_lengths, ind] = C2 * np.log(
                        np.exp(run_length_distr + C1)
                    )
            else:
            ...
            if mark_median:
                med = np.median(
                    run_length_distr_copy[np.where(run_length_distr_copy > -np.inf)]
                )
        ...
        if mark_median:
            ax.plot(
                x_axis[:up_to],  # np.linspace(start,stop,T_),
                (median)[:up_to],
                color=self.median_color,  # linewidth = 1,
                linestyle=(0, (3, 1, 1, 1)),
                linewidth=2.0,
            )
        if mark_max:
            ...
            ax.plot(
                x_axis[:up_to],  # np.linspace(start,stop,T_),
                (maxima)[:up_to],
                color=mark_max_color,  # linewidth = 1,
        ...
        im = ax.imshow(
            pdfs,  # extent=(0,T, T,0),#np.amax(r_max + self.cushion),0 ),#,
            interpolation=None,
            cmap="gray_r",
            norm=LogNorm(),
            aspect=aspect_ratio,
                    ...
                    if m == CP_ind and m not in CP_exclude_indices:
                        if CP_loc <= time_range[-1] and CP_loc >= time_range[0]:
                            ...
                            if CP_ind not in CP_indices_until_now:
                                ...
                                handle = ax.axvline(
                                    x=CP_loc,
                                    color=custom_colors[relevant_index],
                                    linestyle=custom_linestyles[relevant_index],
                                    linewidth=custom_linewidth,
                                    alpha=CP_transparence,
                                )
        ...
        if show_real_CPs:
            ...
            for CP_loc, CP_ind, CP_lab in zip(
                ...
                if CP_loc <= time_range[-1] and CP_loc >= time_range[0]:
                    CP_loc = ((CP_loc - time_range[0]) / T_) * (
                        stop - start
                    ) + start  # -time_range[0] #carry CP forward
                    handle = ax.axvline(
                        x=CP_loc,
                        color=custom_colors[CP_ind],
        ...
        if CP_legend:
            ...
            for color in additional_legend_colors:
                additional_handles.append(
                    plt.Line2D((0, 1), (0, 0), color=color, linewidth=custom_linewidth)
                )  # , marker='-',
                # linestyle=''))
        ...
        if print_colorbar:
            ...
            if log_format and (C1 != 0.0 or C2 != 1.0):
                ...
                colbar.set_ticks(
                    theticks
                )  # pow(10,1), pow(10,2), pow(10,3), pow(10,4), pow(10,5)])  #this color bar gives us the gradient of the rl distro
        ...
        if not event_time_list is None and not label_list is None:
            ...
            for event, label in zip(event_time_list, label_list):
                ...
                if (
                    (not arrows_setleft_indices is None)
                    and (not arrows_setleft_by is None)
                    and (not zero_distance is None)
                    and count_setlefts < len(arrows_setleft_indices)
                ):
                ...
                ax.annotate(
                    label,
                    fontsize=number_fontsize,
                    xy=(event, arrow_distance),
                    xycoords="data",
                    xytext=(event_, -arrow_length),
                    textcoords="data",
                    arrowprops=dict(
                        arrowstyle="->",
                        linewidth=arrow_thickness,
    ...
    def plot_model_and_run_length_distr(
        self,
        print_plt=True,
        time_range=None,
        show_MAP_CPs=True,
        ...
        r_max = 0
        storage_all_retained_run_lengths = self.results[
            self.names.index("all retained run lenghts")
        ]
        for run_lengths in storage_all_retained_run_lengths:
            r_max = max(r_max, np.max(run_lengths))
        ...
        storage_model_and_run_length_log_distr = self.results[
            self.names.index("all model and run length log distributions")
        ]
        T_ = np.size(time_range)  # T_ = T if time_range None
        ...
        for m in range(0, M):
            ...
            for t in range(offset, T_rl):
                ...
                if m < storage_model_and_run_length_log_distr[t][:, :].shape[0]:
                    ...
                    the log-format for this!)"""
                    if not log_format:
                        run_length_distr = storage_model_and_run_length_log_distr[t][
                            m, :
                        ]
                    if mark_median:
                        ...
                        entries in the run length posterior)"""
                        med = np.median(
                            run_length_distr[np.where(run_length_distr > -np.inf)]
                        )
        ...
        for m in range(0, M):
            ...
            plt.imshow(pdfs[m], cmap="gray_r", norm=LogNorm())  # pdfs[m,:,:],
            if mark_median:
...
#    for i in range(0, L):
#        myA[i] = logsumexp(non_inf_rld[:(i+1)])

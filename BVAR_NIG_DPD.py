...
class BVARNIGDPD(BVARNIG):
    ...
    def alpha_param_gradient_computation(
        ...
        if self.alpha_param_learning:
            ...
            self.model_log_evidence_p_eps = logsumexp(joint_log_probs)
            ...
            """STEP 3.2: as before"""
            self.model_log_evidence_m_eps = logsumexp(joint_log_probs)
        ...
        if num_obs > 0:
            ...
            if return_sum:
                if not log_sum:
                    ...
                    sum_1_log, sign = logsumexp(  # E2der_m_ba * (
                        a=(
                            np.log(np.abs(E3))
                    ...
                    if np.size(Klarger0) > 0:
                        ...
                        exact_der_part_log, sign_edpl = logsumexp(
                            a=(
                    ...
                    if np.size(Ksmaller0):
                        eps = pow(10, -5)
                        diff_log, diff_log_sign = logsumexp(
                            a=np.array(
                                [
                                    (an + eps + 0.5 * d * alpha)
                        ...
                        numerical_der_part_log, sign_ndpl = logsumexp(
                            a=(
    ...
    def get_grad_L(params, *args):
        ...
        if num_obs > 0:
            ...
            if return_sum:
                ...
                sum_sum, sum_sum_sign = logsumexp(

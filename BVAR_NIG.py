...
class BVARNIG(ProbabilityModel):
    ...
    def post_QR_updates(self, t):
        ...
        run_length_num = self.retained_run_lengths.shape[0]
        for r in range(0, run_length_num):
            self.M_inv_2_rt[r, :, :] = linalg.solve_triangular(
                a=self.R_rt[r, :, :],
    ...
    def get_one_step_ahead_log_loss_derivatives_power_divergence(self):
        ...
        f_1_log = (1.0 + self.alpha_rld) * (
            special.gammaln(0.5 * (nu_1_vec + p)) - special.gammaln(0.5 * nu_1_vec)
        )
        f_2_log = special.gammaln(0.5 * (nu_2_vec)) - special.gammaln(
            0.5 * nu_2_vec + p
        )
        ...
        expr_1 = scipy.special.gammaln(0.5 * (nu_1_vec + p)) - scipy.special.gammaln(
            0.5 * nu_1_vec
        )
        ...
        f_2_der_log, f_2_der_sign = logsumexp(
        ...
        log_integral_derivatives_val, log_integral_derivatives_sign = logsumexp(
       ...
       where we have computed Int_der(alpha) in log form in STEP 2."""
        ...
        expr_1_val, expr_1_sign = logsumexp(
        ...
        expr_2_val, expr_2_sign = logsumexp(
            a=np.array([expr_2_A, expr_2_B]),
    ...
    def objective_optimization(x, *args):
        ...
        evaluation_objective = logsumexp(MVSt_log_densities + run_length_distro)

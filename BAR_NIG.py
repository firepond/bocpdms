...
class BARNIG(ProbabilityModel):
    ...
    def __init__(
        ...
        S1,
        S2,
        prior_mean_beta_list=None,
        prior_var_beta_list=None,
        prior_mean_scale_list=None,
        prior_var_scale_list=None,
        ...
        for location in range(0, self.S1 * self.S2):
            ...
            self.location_models.append(
                BVARNIG(
                    prior_a_list[location],
                    prior_b_list[location],
                    S1=1,
                    S2=1,
    ...
    def initialization(
        ...
        for lag, location in zip(self.lags_list, range(0, self.S1 * self.S2)):
            ...
            self.location_models[location].initialization(
                X_endo=X_endo_loc,
                X_exo=None,
                Y_2=Y2[location],
                X_exo_2=None,
    ...
    def evaluate_predictive_log_distribution(self, y, t):
        ...
        for location in range(0, self.S1 * self.S2):
            log_densities += self.location_models[
                location
    ...
    def evaluate_log_prior_predictive(self, y, t):
        ...
        prior_prob = 0
        for location in range(0, self.S1 * self.S2):
            prior_prob += self.location_models[location].evaluate_log_prior_predictive(
                y[location], t
            )
        return prior_prob
    ...
    def get_posterior_expectation(self, t, r_list=None):
        ...
        for location in range(0, self.S1 * self.S2):
            ...
            post_mean[:, location] = (
                ...
                .get_posterior_expectation(t)
                .reshape(num_retained_run_lengths)
            )
    ...
    def get_posterior_variance(self, t, r_list=None):
        ...
        num_retained_run_lengths = np.size(self.retained_run_lengths)
        post_var = np.zeros(
            (num_retained_run_lengths, self.S1 * self.S2, self.S1 * self.S2)
        )

        """Note: This makes the diag non-zero and the rest 0"""
        for location in range(0, self.S1 * self.S2):
            post_var[:, location, location] = (
                self.location_models[location]
                .get_posterior_variance(t)
                .reshape(num_retained_run_lengths)
            )
